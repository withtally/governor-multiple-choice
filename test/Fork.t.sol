...
contract ForkTest is Test {
    ...
    function setUp() public {
        ...
        try vm.envString("MAINNET_RPC_URL") returns (string memory mainnetRpcUrl) {
            ...
            // 1. Deploy Timelock
            address[] memory proposers = new address[](1); proposers[0] = address(0); // Placeholder
            address[] memory executors = new address[](1); executors[0] = address(0); // Anyone
            ...
            forkGovernor = new GovernorCountingMultipleChoice(
                IVotes(UNI_TOKEN), 
                forkTimelock, 
                "ForkGovernorUNI"
            );
    ...
    function testFork_ProposeVoteStateAndTargetInteraction() public {
        ...
        address[] memory targets = new address[](1); targets[0] = UNI_TOKEN; // Target the live UNI contract
        ...
        string[] memory options = new string[](2); options[0] = "Approve Transfer"; options[1] = "Reject Transfer";
        ...
        assertEq(uint256(forkGovernor.state(proposalId)), uint256(IGovernor.ProposalState.Pending), "Fork State Pending");
        ...
        if (shouldSucceed) {
            assertEq(uint256(forkGovernor.state(proposalId)), uint256(IGovernor.ProposalState.Succeeded), "Fork State should be Succeeded");
            
            // --- Queue & Execute (if Succeeded) ---
            bytes32 operationId = forkTimelock.hashOperationBatch(targets, values, calldatas, bytes32(0), descriptionHash);
            forkGovernor.queue(targets, values, calldatas, descriptionHash);
            assertEq(uint256(forkGovernor.state(proposalId)), uint256(IGovernor.ProposalState.Queued), "Fork State Queued");
            ...
            assertEq(uint256(forkGovernor.state(proposalId)), uint256(IGovernor.ProposalState.Defeated), "Fork State should be Defeated (Quorum Fail)");
